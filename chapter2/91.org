...
(define (mul-term-list-by-term l t)
  (let ((o (order t))
        (c (coeff t)))
    (define (op x)
      (cons (add (order x)
...
(define (sub-term-lists l1 l2)
  (term-list
   (contents (sub (attach-tag (cons 'sparse (cons 'x l1)))
                  (attach-tag (cons 'sparse (cons 'x l2)))))))
  ...
  (if (empty-termlist? L1)
      ...
      (let ((t1 (first-term L1))
        ...
        (if (> (order t2) (order t1))
            ...
            (let* ((new-c (div (coeff t1) (coeff t2)))
                   (new-o (- (order t1) (order t2)))
                   (new-term (cons new-o new-c)))
              (let ((rest-of-result
                     (div-terms (sub-term-lists L1
                                                (mul-term-list-by-term L2 new-term))
                                L2)))
                (cons (cons new-term (car rest-of-result))
                      (cdr rest-of-result))))))))

(put 'div '(poly) (lambda (x y) (attach-tag 'poly
                                            (div-terms (term-list (contents x))
                                                       (term-list (contenty y))))))
#+end_src
